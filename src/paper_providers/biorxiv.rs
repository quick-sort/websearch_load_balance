...
#[async_trait]
impl PaperSearchProvider for BiorxivProvider {
    fn name(&self) -> &str {
        "biorxiv"
    }
    ...
    async fn search(
        ...
        // bioRxiv API uses date range; search last 30 days
        ...
        if !resp.status().is_success() {
            return Err(WebSearchError::ProviderError(
                ...
                "bioRxiv API error".into(),
            ));
        }

        let data: BiorxivResponse = resp.json().await?;
...
mod tests {
    ...
    fn test_provider_name() {
        let p = BiorxivProvider::new();
        assert_eq!(p.name(), "biorxiv");
    ...
    #[tokio::test]
    #[ignore] // hits real bioRxiv API
    async fn test_search_integration() {
        let p = BiorxivProvider::new();
        let result = p.search("neuroscience", 3).await;
        assert!(result.is_ok(), "search failed: {:?}", result);
        let resp = result.unwrap();
        for paper in &resp.papers {
            assert_eq!(paper.source, "biorxiv");
